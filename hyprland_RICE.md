...
-- lisgd -d /dev/input/event13 -g "1,RL,_,_,R,notify-send next worksapce && hyprctl dispatch movetoworkspace +1 " 右往左滑动下一个工作区
...
将锁屏快捷键替换为 'init.conf:SCREEN_LOCK=loginctl lock-session'，由 'hypridle' 监听 'dbus/sysd lock' 然后执行 'lock_cmd' 和 'on_lock_cmd' 事件

```hypridle.conf
general {
    lock_cmd = pidof hyprlock || hyprlock
    on_lock_cmd = sleep 3 && hyprctl dispatch dpms off
}
```

NOTE: 如果遇到 HDMI 外置显示器 dpms 失效问题，可以尝试关闭显示器的自动选择输入功能
