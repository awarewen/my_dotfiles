...
| 修正项 | 依据来源 |
|--------|----------|
| O 键透明切换（setprop 语法） | Hyprland 0.53.x+ 变更：`hyprctl setprop activewindow` 返回 unknown request；正确用法为 `hyprctl dispatch setprop active opaque toggle`。参考：Hyprland wiki Using hyprctl、GitHub Discussions #12861 / #1779。 |
| resizeactive 逗号语法 | Hyprland wiki → Configuring → Dispatchers：`resizeactive` 参数格式与 dispatch 用法。 |
| 其余注释与重复绑定 | 对照 `custom/keybinds.conf` 与 `custom/env.conf` 实际绑定与变量使用情况核对。 |

**文档生成**

- 由 **Cursor**（Agent / Composer）根据对话中的 keybinds 检查与修改结果整理生成。  
- 日期：**2025-02-03**。
