...
# 临时文件路径定义
TEMP_WALLPAPER_FILE="$HOME/.cache/wallpaper_path.tmp"
...
process_and_cache_image() {
  local source_path="$1"
  local target_path="$CACHED_WALLPAPER_FILE"
  ...
  if [[ "$img_width" -lt "$max_width" || "$img_height" -lt "$max_height" ]]; then
    ...
    # 使用convert命令调整尺寸并复制到缓存
    if command -v convert &>/dev/null; then
      convert "$source_path" -resize "${new_width}x${new_height}^" -gravity center -extent "${max_width}x${max_height}" "$target_path"
      echo "图片已调整尺寸并缓存: ${img_width}x${img_height} -> ${max_width}x${max_height}"
    else
      # 如果没有convert命令，直接复制原图
      cp "$source_path" "$target_path"
      echo "警告: 未找到convert命令，直接复制原图"
    fi
  else
    # 尺寸足够，直接复制
    cp "$source_path" "$target_path"
    echo "图片尺寸足够，直接缓存: ${img_width}x${img_height}"
  fi
...
save_cached_wallpaper_path_to_temp() {
  ...
  local cached_path="$CACHED_WALLPAPER_FILE"
  ...
  while [[ $# -gt 0 ]]; do
    ...
    --noswitch)
      ...
      if [ -n "$imgpath" ] && [ -f "$imgpath" ]; then
        if [ -f "$CACHED_WALLPAPER_FILE" ]; then
          imgpath="$CACHED_WALLPAPER_FILE"
        fi
      fi
      shift
      ;;
    ...
    --show-cache)
      if [ -f "$CACHED_WALLPAPER_FILE" ]; then
        echo "缓存文件存在: $CACHED_WALLPAPER_FILE"
        if command -v identify &>/dev/null; then
          cached_size=$(identify -format "%wx%h" "$CACHED_WALLPAPER_FILE" 2>/dev/null)
          echo "缓存文件尺寸: $cached_size"
        fi
      else
        echo "缓存文件不存在"
      fi
  ...
  if [[ -z "$imgpath" && -z "$color_flag" && -z "$noswitch_flag" ]]; then
    ...
    if [ -n "$new_imgpath" ] && [ -f "$new_imgpath" ]; then
      ...
      if [ -n "$temp_imgpath" ] && [ -f "$temp_imgpath" ]; then
        ...
        # 比较文件名，如果相同则使用临时文件，否则使用新文件
        if [ "$new_filename" = "$temp_filename" ]; then
          # 如果文件名相同，我们需要从配置文件获取原始路径，而不是使用临时文件中的缓存路径
          ...
          if [ -n "$config_imgpath" ] && [ -f "$config_imgpath" ]; then
            ...
            echo "文件名相同，使用配置文件中的原始壁纸: $config_imgpath"
          else
            # 如果配置文件中没有原始路径，使用新选择的文件
            imgpath="$new_imgpath"
            echo "文件名相同但配置文件无原始路径，使用新选择的壁纸: $new_imgpath"
          fi
        else
          imgpath="$new_imgpath"
