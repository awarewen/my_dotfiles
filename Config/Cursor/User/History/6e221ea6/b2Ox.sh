...
# 性能优化选项
FAST_PROCESSING="${FAST_PROCESSING:-false}"  # 设置为true可以启用更快的图片处理
...
process_and_cache_image() {
  ...
  if [[ "$img_width" -lt "$max_width" || "$img_height" -lt "$max_height" ]]; then
    ...
    # 使用magick命令调整尺寸并复制到缓存（优化版本）
    if command -v magick &>/dev/null; then
      if [[ "$FAST_PROCESSING" == "true" ]]; then
        # 快速处理模式：使用最快的设置
        # -filter Box: 最快的插值算法
        # -quality 75: 较低质量但更快的处理
        # -strip: 移除元数据
        magick "$source_path" -limit memory 128MB -limit map 256MB -filter Box -resize "${new_width}x${new_height}^" -gravity center -extent "${max_width}x${max_height}" -quality 75 -strip "$target_path"
        echo "快速处理模式: 图片已调整尺寸并缓存" >&2
      else
        # 标准处理模式：平衡质量和速度
        # -filter Triangle: 平衡的插值算法
        # -quality 85: 良好质量
        # -strip: 移除元数据以减小文件大小
        magick "$source_path" -limit memory 256MB -limit map 512MB -filter Triangle -resize "${new_width}x${new_height}^" -gravity center -extent "${max_width}x${max_height}" -quality 85 -strip "$target_path"
      fi
      ...
      if [ -f "$target_path" ] && magick identify "$target_path" &>/dev/null; then
        ...
        # 备用方法：使用最简化的resize
        magick "$source_path" -limit memory 64MB -limit map 128MB -filter Box -resize "${max_width}x${max_height}" -quality 70 "$target_path"
        if [ ! -f "$target_path" ] || ! magick identify "$target_path" &>/dev/null; then
...
main() {
  ...
  if [[ -z "$imgpath" && -z "$color_flag" && -z "$noswitch_flag" ]]; then
    ...
    # 移除可能的引号
    new_imgpath=$(echo "$new_imgpath" | sed 's/^["'\'']*//;s/["'\'']*$//')
    # 如果是相对路径，转换为绝对路径
    if [[ "$new_imgpath" != /* ]]; then
