...
process_and_cache_image() {
  ...
  if [[ "$img_width" -lt "$max_width" || "$img_height" -lt "$max_height" ]]; then
    ...
    if command -v magick &>/dev/null; then
      # 使用优化的magick命令参数，提高处理速度
      # -limit memory 256MB -limit map 512MB: 限制内存使用
      # -filter Triangle: 使用更快的插值算法
      # -quality 85: 降低质量以提高速度（85仍然很好）
      # -strip: 移除元数据以减小文件大小
      ...
      if [ -f "$target_path" ] && magick identify "$target_path" &>/dev/null; then
        ...
        # 备用方法：使用更简单的resize，进一步优化
        magick "$source_path" -limit memory 256MB -limit map 512MB -filter Box -resize "${max_width}x${max_height}^" -gravity center -extent "${max_width}x${max_height}" -quality 80 -strip "$target_path"
        if [ ! -f "$target_path" ] || ! magick identify "$target_path" &>/dev/null; then
